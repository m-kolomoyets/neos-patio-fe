...
type UseHomeViewArgs = {
    ...
    storageId: string;
    /** Smooth in-place orbit snap (fit-framed) — the shared cube move. */
 ...
 * returns to default. All moves go through the shared `snapTo`, so Home lands
 * with the same smooth in-place orbit as a cube face click. Home is viewport
 * state, never the undo reducer.
 */
export const useHomeView = ({ storageId, snapTo, readOrientation, referenceRange }: UseHomeViewArgs) => {
    const storageKey = `${HOME_STORAGE_PREFIX}${storageId}`;

    const defaultHome = useMemo<HomeView>(() => {
        return { bearing: DEFAULT_BEARING, pitch: DEFAULT_PITCH, range: referenceRange };
    }, [referenceRange]);

    /** Snap to the saved Home (falling back to the editor default). */
    const goHome = useCallback(() => {
        snapTo(readStoredHome(storageKey) ?? defaultHome);
    }, [snapTo, storageKey, defaultHome]);
    ...
    /** Forget the saved Home and snap back to the editor default. */
    const resetHome = useCallback(() => {
        try {
            localStorage.removeItem(storageKey);
        } catch {
            // Ignore storage errors; the snap-to-default below still runs.
        }
        snapTo(defaultHome);
    }, [snapTo, storageKey, defaultHome]);
