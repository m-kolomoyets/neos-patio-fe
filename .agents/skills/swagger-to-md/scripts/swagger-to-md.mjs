...
const CANDIDATE_SUFFIXES = ['', '/swagger.json', '/openapi.json', '/v3/api-docs', '/api-docs'];
...
function render(spec) {
    ...
    if (spec.servers?.length) {
        idx.push(
            ...
            '## Servers',
            '',
            ...spec.servers.map((s) => `- \`${s.url}\`${s.description ? ` — ${s.description}` : ''}`)
        );
    } else if (spec.host) {
                ...
                for (const [mt, def] of Object.entries(content)) {
                    if (def.schema) {
                        ...
                        lines.push(
                            ...
                            '',
                            '  ```json',
                            indent(JSON.stringify(resolved, null, 2), '  '),
                            '  ```'
                        );
                    }
                }
...
function schemaToExample(schema, ctx, seen = new Set()) {
    ...
    if (schema.allOf) {
        ...
        if (schema.properties)
            Object.assign(
                merged,
                schemaToExample({ type: 'object', properties: schema.properties, required: schema.required }, ctx, seen)
            );
