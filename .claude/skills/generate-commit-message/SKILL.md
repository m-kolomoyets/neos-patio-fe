...
description: >
  ...
  ticket IDs (e.g. ISORA-1234) from the current branch and appends a `Refs:` footer.
  Validates the final message with `npx --no -- commitlint --edit` before presenting.
...
7. Validate by piping the message through `npx --no -- commitlint --edit` (see [Validation](#validation)).
...
The repo already gates commits via [`.husky/commit-msg`](../../../.husky/commit-msg) running `npx --no -- commitlint --edit $1`. Reuse that exact tool for pre-flight validation:

```bash
# Write candidate message to a temp file, then validate.
printf '%s\n' "$MESSAGE" > /tmp/commit-msg-candidate
npx --no -- commitlint --edit /tmp/commit-msg-candidate
```
...
With auto-detected ticket footer (branch `feat/ISORA-1234-bulk-archive`):
